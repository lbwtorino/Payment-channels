...
contract Channel2 {
    ...
    function terminate(uint A, uint B, uint round, uint random, byte[] _v, bytes32[] _r, bytes32[] _s) {
        assert(flag == 1);
        // test4 = data.verify2identity(A, B, round, random, _v[1],  _r[1], _s[1]);
        // test5 = data.verify2identity(A, B, round, random, _v[0],  _r[0], _s[0]);
    ...
    function challenge(bytes32 Hvalue, uint round, byte _v, bytes32 _r, bytes32 _s) {
        ...
        if ((flag == 0 && round >= latestRound) || isRespond == false){
            tower.withdraw(msg.sender, 48);
            test3 = 888;
        }
        if (percentage > 0){
            tower.withdraw(msg.sender, percentage);
        }  
    }
}
